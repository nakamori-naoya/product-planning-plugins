...
同じpackage内の`product-north-star`と、外部の`grill@grill`、`write-doc@write-doc`に依存する。別repositoryには公開playbook packageだけで依存し、その内部機能名へは依存しない。versionは固定せず、解決先のmanifest identityと必要なskillまたはplaybookを検査する。
