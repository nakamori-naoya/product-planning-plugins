...
- **成果を左右する6問に入るか。** 候補のうち、回答によって受益者・望む状態・約束する価値・判断原則のいずれかが別物になるものを優先し、`questions` は最大6問に絞る。表現や粒度しか変わらない論点、資料を見てから確かめれば足りる論点は問わず、推奨を仮置きした未決にする。
...
1. **価値判断を確かめる（`settle-north-star`）。** `grill` の公開契約の入力object（`contract: grill/grill`、`version: 1`、`topic`、`context` の `purpose` / `audience` / `boundary`、判断基準で6問以内に絞った `questions`（推奨付き。無ければ `[]`）、既知の材料の `grounding`。保存も求められた場合だけ `output_to`）を公開Skill `grill:grill` へ直接渡し、一問ずつの対話を同じ会話で進める。返ったobjectの `status` が `completed` なら `decisions` と `open_questions` を使い、`failed` なら `reason` を報告して止まる。契約IDや版の不一致、`rationale` の無い決定、`open` / `withdrawn` 以外の状態、配列の欠落や `null` は不正な結果として止まる。空配列は合法である。2回目の `grill` は、利用者が求めた場合か、決定なしでは本文を完成できない場合だけ行う。
