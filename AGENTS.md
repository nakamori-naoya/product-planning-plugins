...
`SKILL.md`、`references/`、`playbook.yml` に実行基盤の配管（`${.`マクロ、同期block、環境変数によるroot解決、設定解決scriptの実行指示、`prepare.sh` / `resolve.sh` の対）を書かない。設定fileを置かず、保存先は公開入力 `document_destination` で受け取る。後片付けは入口の `scripts/cleanup.py` で、自分が所有するrun専用directory内の中間成果物だけを削除する。
