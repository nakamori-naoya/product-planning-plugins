...
for expr in '.requires = [.requires[] | select(.plugin != "grill")]' '.requires = [.requires[] | select(.plugin != "write-doc")]' '.requires[2].marketplace="product-planning"' '.steps[0] |= (del(.playbook) + {skill: "grill"})' '.steps[0].provides=["decisions","grounded_input"]' ".steps[1].script=\"scripts/$wd_internal_cleanup.py\"" ".steps[4] |= (del(.playbook) + {skill: \"$wd_internal_writer\"})" '.steps[4].input={}' ".steps[5] |= (del(.script) + {skill: \"$wd_internal_cleanup\"})" '.document_type="concept"' '.contract.forbidden_sections=[]'; do
  yq -o=json '.' "$NORTH_ROOT/playbook.yml" | jq "$expr" > "$TMP/north-mutated.json"
...
for expr in '.requires = [.requires[] | select(.plugin != "grill")]' '.requires = [.requires[] | select(.plugin != "write-doc")]' '.requires[4].marketplace="product-planning"' '.steps[1].needs=[]' '.steps[2] |= (del(.playbook) + {skill: "grill"})' '.steps[2].provides=["decisions","unresolved","grounded_strategy"]' '.steps[6].needs=["product_strategy_path"]' ".steps[7] |= (del(.playbook) + {skill: \"$wd_internal_writer\"})" '.steps[7].input={}' ".steps[8] |= (del(.script) + {skill: \"$wd_internal_cleanup\"})" '.document_type="concept"' '.contract.critique_verdicts=["pass","revise"]'; do
  yq -o=json '.' "$STRATEGY_ROOT/playbook.yml" | jq "$expr" > "$TMP/strategy-mutated.json"
...
# 公開YAMLのdocument_destinationがdocument工程まで届くことを、同じ固有validator経路で守る。
for playbook_root in "$NORTH_ROOT" "$STRATEGY_ROOT"; do
  ...
  bash "$playbook_root/scripts/validate-config.sh" "$TMP/$playbook_name-destination.json" \
    || ng "$playbook_nameの公開保存先接続"
  jq '(.steps[] | select(.id=="document").needs) |= map(select(.!="document_destination"))' \
    "$TMP/$playbook_name-destination.json" > "$TMP/$playbook_name-no-destination-need.json"
  expect_fail bash "$playbook_root/scripts/validate-config.sh" "$TMP/$playbook_name-no-destination-need.json"
  jq '.inputs |= map(select(.!="document_destination"))' \
    "$TMP/$playbook_name-destination.json" > "$TMP/$playbook_name-no-public-destination.json"
  expect_fail bash "$playbook_root/scripts/validate-config.sh" "$TMP/$playbook_name-no-public-destination.json"
  jq '.inputs += ["output_target"]' \
    "$TMP/$playbook_name-destination.json" > "$TMP/$playbook_name-old-output-target.json"
  expect_fail bash "$playbook_root/scripts/validate-config.sh" "$TMP/$playbook_name-old-output-target.json"
done
