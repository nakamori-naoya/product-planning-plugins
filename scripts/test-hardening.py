...
class Hardening(unittest.TestCase):
    ...
    def test_semantic_runner_records_assessment_but_only_operational_errors_fail(self):
        ...
        self.assertEqual(result.returncode,0);report=json.loads(out.read_text());record=report['records'][0];self.assertEqual(report['schema'],2);self.assertEqual(record['status'],'recorded');self.assertFalse(record['assessment']['all_criteria_passed'])
